...
def run():
    with open('constituency.json', 'r') as f:
        ...
        district_sheet.append(['id','code','state_code', 'name','link', 'no_of_constituencies'])

        constituency_sheet = workbook.create_sheet("Constituencies")
        constituency_sheet.append(['id','code','district_code', 'name','link'])
        for _, state_data in data.items():
            ...
            for _, district_data in districts.items():
               ...
               district_sheet.append([str(uuid4()),district_code,state_code, district_data['name'], district_data['link'], len(constituencies.items())])
               for _, constituency_data in constituencies.items():
                    constituency_sheet.append([str(uuid4()),generate_unique_code(), district_code, constituency_data['name'], constituency_data['link']])
